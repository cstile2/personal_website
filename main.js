...
void main() {
    ...
    vec2 _p = p/70.0 + n.xy * 1.5;
    vec3 a = vec3(0.05,0.12, 0.25);
    vec3 b = vec3(0.5,0.5,0.5);
    vec3 col = mix(a,b, sin(_p.x) * 0.5 + 0.5 + _n.w * 0.04) * 0.5;
    col += vec3(sin(_p.x)) * -0.23;
    // vec3 col = vec3(s(_p.x), s(_p.x+2.0*3.14159/3.0), s(_p.x+4.0*3.14159/3.0));
    col += _n.www * 0.002;

    vec3 L = normalize(vec3(1,0,0));
    vec3 R = normalize(vec3(0,1,1));
    vec3 col2 = dot(col, L) * L + dot(col, R) * R;
...
// const srcs = ["assets/videos/beach.mp4", "assets/videos/canada.mp4", "assets/videos/clouds.mp4", "assets/videos/mountains.mp4", "assets/videos/stars.mp4"]
